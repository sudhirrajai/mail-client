...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showSuccess('Account Created', `Welcome to Mosaic Mail, ${res.user.name}!`)
      router.replace('/')
    } catch (err: any) {
      showError('Registration Failed', err.message || 'Could not register user account.')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center bg-background p-4 text-foreground overflow-hidden">
      ...
      <div className="absolute -bottom-40 -left-40 size-96 rounded-full bg-emerald-500/10 blur-3xl" />
      ...
      <div className="relative w-full max-w-md rounded-3xl border border-border/80 bg-card/80 p-8 shadow-2xl backdrop-blur-xl">
        ...
        <div className="mb-8 flex flex-col items-center text-center">
          ...
          <h1 className="text-2xl font-bold tracking-tight">Create Webmail Account</h1>
          <p className="mt-1.5 text-xs text-muted-foreground">Register your profile to start connecting mailboxes</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            {loading ? <Loader2 className="size-4 animate-spin" /> : <>Register Account <ArrowRight className="size-4" /></>}
          </button>
        </form>
        ...
        <div className="mt-8 border-t border-border/70 pt-6 text-center text-xs text-muted-foreground">
          <p>
            Already registered?{' '}
            <Link href="/login" className="font-semibold text-primary underline hover:opacity-90">
